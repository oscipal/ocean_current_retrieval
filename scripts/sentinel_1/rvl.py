...
def compute_gamma_ambiguity(
    ...
    Parameters
    ----------
    aap       : AzimuthAntennaPattern  (from aux_files.parse_aux_cal)
    prf       : float  [Hz]
...
def _fm_rate_at_burst(annot: S1Annotation, burst_idx: int) -> np.ndarray:
    ...
    Uses the nearest azimuthFmRate record to the burst time.  k_a is
...
def deramp_burst(
    ...
    # the boundary between the two middle samples. A half-line offset leaves a
    # residual linear phase term after deramping, which appears as a per-burst
    # Doppler bias and can create seams at burst boundaries.
    t_az = (
